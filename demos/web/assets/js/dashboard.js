	...
	socket.onmessage = function (e) {
		...
		j = JSON.parse(e.data)
			if (j.type == "PEOPLE") {
				...
				if (minmaxCameraIP.cameraIPs.indexOf(j.data.cameraIP) === -1) 
...
function refreshDataCommon() {
	data_starttimeXnum = starttimeXnum(minmaxCameraIP.min);
	data_starttimeXcameraIPXnum = starttimeXcameraIPXnum(minmaxCameraIP.min, minmaxCameraIP.cameraIPs);
}
